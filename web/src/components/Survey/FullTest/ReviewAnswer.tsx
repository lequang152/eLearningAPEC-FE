    ...
    if(GlobalVariableInstance.getScoringType() === 'scoring_with_answers') {
      return (
        <div className="container mt-10">
          <div className='text-center text-4xl font-bold text-cyan-800 mb-5'>
            Your score is:
          </div>
          <div className='flex flex-col gap-3 items-center screenReview:flex-row screenReview:justify-around mb-6'>
            ...
            <div className='text-green-700 bg-green-100 font-bold text-2xl border-8 
            ...
            <div className='border-4 w-32 h-32 flex flex-col items-center justify-center rounded-full'>
              ...
              <div className='text-cyan-900 font-bold'>{`${minutes > 0 ? minutes : 0} : ${seconds > 0 ? seconds : 0}`}</div>
                      ...
                      <div key={index} className='mb-16'>
                        ...
                        <ul className='grid sm:max-lg:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4'>
                          {questions.map((question, index) => {
                            ...
                            return(
                              <li className="flex items-center px-3 py-2 flex-wrap border rounded " key={index}>
                                <span className={`font-bold rounded-full bg-green-600 text-white h-7
                                  ...
                                  <span className="mr-2">
                                    {GlobalVariableInstance.getReviewAnswerData()?.detail[id]?.userAnswer === undefined
                                      ? ''
                                      : GlobalVariableInstance.getReviewAnswerData().detail[id]?.userAnswer.map((item: string, index: number) => (
                                  ...
                                  {
                                    GlobalVariableInstance.getReviewAnswerData().detail[id]?.correct ? <DoneIcon sx={{color: 'green'}}/> 
